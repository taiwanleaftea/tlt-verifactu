...
class CertificateManager
{
     ...
     */
    public static function getCertificate(string $certificate, string $password = ''): string|false
    {
        if (!file_exists($certificate)) {
            throw new CertificateException('Certificate not found: $certificate');
        }

        $ext = strtolower(pathinfo($certificate, PATHINFO_EXTENSION));
        $certContent = file_get_contents($certificate);
        ...
        if ($ext === 'pem') {
            ...
            throw new CertificateException('No X.509 certificate found in PEM file.');
        }

        if ($ext === 'pfx' || $ext === 'p12') {
            // Convertir PFX a PEM (intenta PHP, hace fallback a CLI con -legacy si hace falta)
            $certs = self::readPkcs12($certificate, $password);
